...
export default function SudokuMenu() {
  ...
  const BOARD_SIZE: { width: string; height: string } = getBoardSize(
    options.SUDOKU_SIZE,
  ...
  function algoSolveSudoku() {
    ...
    console.log(
      `%csolving took ${Date.now() - startTime}ms`,
      "color: #7fffd4; font-size: 2rem; font-weight: 600; text-shadow: .25rem .25rem .5rem #f0f8f5"
    );
  }
  ...
  const { isLoading: isSudokuLoading, mutate: getSudokuBoard } = useMutation(
    getSudokuFromImage,
    {
      onSuccess: (res) => {
        if (!res) return;
  ...
  function getHint() {
    ...
    newBoard[randomRow][randomColumn].value =
      initialBoardInfo.filledBoard[randomRow][randomColumn];
  ...
  return (
    <StyledSudokuMenu sudokuSize={options.SUDOKU_SIZE} ref={menuRef}>
      {gameWon && !finishedWithButton ? (
        ...
        <SudokuBoard width={BOARD_SIZE.width} height={BOARD_SIZE.height} />
      )}
